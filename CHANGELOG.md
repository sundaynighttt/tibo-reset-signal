...
- iPhone 전용 SwiftUI 앱과 WidgetKit 홈 화면 위젯을 추가하고 TestFlight 빌드 `0.1.0 (7)`까지 배포했습니다.
...
- Shipped the iPhone-only TestFlight build `0.1.0 (7)` with a large solid-color signal, a balanced title/refresh header, a left-aligned score, latest evidence, and the original-post link.
