...
struct SignalGuideView: View {
    var body: some View {
        List {
            ...
            Section("위젯과 갱신") {
                Text("홈 화면을 길게 누른 뒤 편집 → 위젯 추가에서 Tibo Reset Signal을 선택하세요. 작은 위젯에서 신호와 최신 근거를 확인하고 새로고침할 수 있습니다.")
                Text("운영 수집기는 약 1시간마다 확인합니다. 새로고침은 이미 공개된 최신 상태를 다시 읽으며, X를 즉시 다시 조회하지 않습니다. 위젯 자동 갱신 시각은 iOS가 결정합니다.")
            }

            Section("출처와 운영") {
                Text("공개된 @thsottiaux 게시물의 리셋 관련 정황을 분석합니다. 앱에는 원문·사진을 복제하지 않고 판정 이유와 원문 링크를 표시합니다.")
